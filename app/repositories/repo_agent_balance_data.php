...
use App\Models\mod_agent_balance_data;
use App\traits\trait_repo_base;

class repo_agent_balance_data
{
    use trait_repo_base;
    ...
    public function __construct(mod_agent_balance_data $mod_agent_balance_data)
    {
        $this->model = $mod_agent_balance_data;
    }
    ...
    public function get_list(array $conds)
    {
        ...
        $is_saturate     = $conds['is_saturate'] ?? null; //是否饱和

        $where = []; //筛选
        $date_start and $where[] = ['date', '>=', $date_start];
        $date_end and $where[] = ['date', '<=', $date_end];
        $agent_id and $where[] = ['agent_id', '=', $agent_id];
        (is_numeric($is_saturate) && $is_saturate == 0) and $where[] = ['remain_balance', '>', 0.00];
        (is_numeric($is_saturate) && $is_saturate == 1) and $where[] = ['remain_balance', '<=', 0.00];
