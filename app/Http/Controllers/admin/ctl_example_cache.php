 ...
 * 文章控制器 (范例) - 使用缓存
 * 默认走php序列化存储
 * Class ctl_example_cache
 * @package App\Http\Controllers\admin
 */
class ctl_example_cache extends Controller
{
    ...
    public function detail(Request $request)
    {
        ...
        $row = Cache::remember($cache_key, $this->cache_time, function() use($id) { //存储在redis里都会被序列化
            $example_info = $this->repo_example->find(['where' => [['id', '=', $id]]]);
            return json_encode($example_info, JSON_UNESCAPED_UNICODE);
        });
    ...
    public function edit(Request $request)
    {
        ...
        //干掉緩存
        $cache_key = sprintf($this->repo_example->detail_cache_key, $id);
        !empty($id) and Cache::forget($cache_key);
