...
/**
 * 渠道代理登入日志控制器
 * Class ctl_agent_login_log
 * @package App\Http\Controllers\admin
 */
class ctl_agent_login_log extends Controller
{
    private $repo_agent_login_log;

    public function __construct(repo_agent_login_log $repo_agent_login_log)
    {
        parent::__construct();
    ...
    public function index(Request $request)
    {
        ...
        $page_size  = $request->input('page_size', $this->repo_agent_login_log->page_size);
        ...
        $rows = $this->repo_agent_login_log->get_list($conds);
    ...
    public function delete(Request $request)
    {
        ...
        $status = $this->repo_agent_login_log->del(['id' => $id]);
        if($status < 0)
        {
            return res_error($this->repo_agent_login_log->get_err_msg($status), $status);
        }
